const summaryConfig = [
  { id: 'total-paper-sections', label: 'Total Paper Sections', valueKey: 'total_sections', iconKey: 'sections' },
  { id: 'total-slides', label: 'Total Slides', valueKey: 'total_slides', iconKey: 'slides' },
  { id: 'matching-sections', label: 'Matching Sections', valueKey: 'matching_sections', iconKey: 'match', valueClass: 'green' },
  { id: 'missing-sections', label: 'Missing Sections', valueKey: 'missing_sections', iconKey: 'missing', valueClass: 'orange' },
  { id: 'extra-topics', label: 'Extra Topics', valueKey: 'extra_topics', iconKey: 'extra', valueClass: 'red' },
]
...
export default function QuickSummary({ report }) {
  const summary = report?.quick_summary || {}
  return (
      ...
      <div className="summary-list">
        {summaryConfig.map((item) => (
          <div className="summary-item" key={item.id} id={`summary-${item.id}`}>
            ...
            <span className={`summary-value ${item.valueClass || ''}`}>{summary[item.valueKey] ?? '–'}</span>
          </div>
        ))}
      </div>
    </div>
